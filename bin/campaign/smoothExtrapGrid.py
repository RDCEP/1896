        ...
        for j in range(len(latidx)):
            ...
            if vmask[lidx1, lidx2]:
                for k in range(len(radii)):
                    inrange = sqrt(wlat * (latdv - lats[lidx1]) ** 2 + wlon * (londv - lons[lidx2]) ** 2) < radii[k]

                    if len(sh) == 2:
                        smoothed = vi[latidxv[inrange], lonidxv[inrange]].mean()
                        vo[lidx1, lidx2] = smoothed
                    elif len(sh) == 3:
                        smoothed = vi[:, latidxv[inrange], lonidxv[inrange]].mean(axis = 1)
                        vo[:, lidx1, lidx2] = smoothed
                    else:
                        smoothed = vi[:, :, latidxv[inrange], lonidxv[inrange]].mean(axis = 2)
                        vo[:, :, lidx1, lidx2] = smoothed
